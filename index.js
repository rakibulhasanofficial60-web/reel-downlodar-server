...
app.get("/api/download", async (req, res) => {
    ...
    try {
        ...
        if (video) {
            res.json({ type: "video", media: video, thumbnail: image });
        } else if (image) {
            res.json({ type: "image", media: image, thumbnail: image });
        } else {
...
app.get('/test', async (req, res) => {
    res.send('Yes, comming');
})

app.listen(3000, () => console.log("downloder server running on port 3000"));